...
def test_hendrycks_math(model, data_path, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1, log_file=None):
    ...
    logger.info('len invalid outputs ==== %d', len(invalid_outputs))
    logger.info('start=== %d, end==== %d', start, end)
    logger.info('length==== %d, acc==== %f', len(results), acc)

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default=0)  # model path
    parser.add_argument("--data_file", type=str, default='dataset/MATH_test.jsonl')  # data path
...
if __name__ == "__main__":
    args = parse_args()
    test_hendrycks_math(model=args.model, data_path=args.data_file, start=args.start, end=args.end, batch_size=args.batch_size, tensor_parallel_size=args.tensor_parallel_size, log_file=args.log_file)
