...
export MODEL_NAME_OR_PATH="meta-llama/Llama-2-7b-hf" # meta-llama/Llama-2-7b-hf meta-llama/Meta-Llama-3-8B # Qwen/Qwen3-8B
...
export OUTPUT_PATH="outputs/metamath-${INIT_WEIGHTS}-${MODEL_NAME}-r${LORA_RANK}"
...
export LOCALHOST="localhost:4,5,6,7"
...
export AVAILABLE_DEVICES="4"
export EVAL_DEVICE="4,5,6,7"

# Logging arguments
export LOG_FILE="logs/metamath/${INIT_WEIGHTS}-${ASTRA_METHOD}-${MODEL_NAME}-r${LORA_RANK}.log"
export LOG_DIR="logs/log_history/metamath/${INIT_WEIGHTS}-${ASTRA_METHOD}-${MODEL_NAME}-r${LORA_RANK}"
export REPORT_TO="wandb"

# Calibration parameters
export CALIBRATION_DATASET="metamath"
export NUM_CALIBRATION_SAMPLES=256
export CALIBRATION_MAX_SEQ_LENGTH=512
export CALIBRATION_DEVICE="cuda:7"
export RANK_ALLOCATION=false
export RANK_PATTERN_PATH="cache/rank_pattern.pt"

# Helper functions
setup_logging() {
    mkdir -p "logs/log_history/metamath"
    mkdir -p "logs/metamath"
    if [ "$INIT_WEIGHTS" == "Full_FT" ]; then
        LOG_DIR="logs/log_history/metamath/${INIT_WEIGHTS}_${MODEL_NAME}"
        LOG_FILE="logs/metamath/${INIT_WEIGHTS}_${MODEL_NAME}.log"
    else
        LOG_DIR="logs/log_history/metamath/${INIT_WEIGHTS}-${ASTRA_METHOD}-${MODEL_NAME}-r${LORA_RANK}"
        LOG_FILE="logs/metamath/${INIT_WEIGHTS}-${ASTRA_METHOD}-${MODEL_NAME}-r${LORA_RANK}.log"
    fi
}
