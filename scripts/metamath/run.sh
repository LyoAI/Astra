#!/bin/bash
source scripts/metamath/params_script.sh
...
echo "Start evaluating GSM8K"
CUDA_VISIBLE_DEVICES=$EVAL_DEVICE python utils/gsm8k_inference.py --model $OUTPUT_PATH --tensor_parallel_size $NUM_GPUS --log_file $LOG_FILE

echo "Start evaluating MATH"
CUDA_VISIBLE_DEVICES=$EVAL_DEVICE python utils/MATH_inference.py --model $OUTPUT_PATH --tensor_parallel_size $NUM_GPUS --log_file $LOG_FILE